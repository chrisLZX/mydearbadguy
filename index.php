...
function writeTweets($my_tweets){
	foreach ($my_tweets  as $my_tweet) {
		$arrTw = '<li>'.$my_tweet->text.' <br />-<i>'.$my_tweet->created_at.'</i></li>';
		echo '<script>$("ul#tweets").append("'.$arrTw.'");</script>';
	}
}
...
{	//Success, redirected back from process.php with varified status.
	...
	$screenname2		= "mydearbadguy";
	$twitterid 			= $_SESSION['request_vars']['user_id'];
	$oauth_token 		= "768175776-F1voaPLT7c3oNNevEoQzE5jjzg6PS892vq2lvjMI";
	$oauth_token_secret = "TlcOf8Qy9ESnmRQyCIdAG64VT9EhJdRkKKnVB3d1JI";
	...
	if(isset($_POST["updateme"])) 
	{
		...
		$my_tweets = $connection->get('statuses/user_timeline', array('screen_name' => $screenname2, 'count' => 5));
		writeTweets($my_tweets);
	}
	...
	echo '</div>';
		...
		//Get latest tweets
		$my_tweets = $connection->get('statuses/user_timeline', array('screen_name' => $screenname2, 'count' => 5));		
		echo '<div class="tweet_list"><strong>Latest Tweets (@mydearbadguy): </strong>';
		echo '<ul id="tweets">';
		echo '</ul></div>';
